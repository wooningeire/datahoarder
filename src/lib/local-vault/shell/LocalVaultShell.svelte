...
import { getErrorMessage } from "./shell-utils.js";
...
const requestState = createShellRequestState({
	clearVaultSearchQuery: () => {
		vaultSearchQuery = '';
	}
});
const storedNotes = createStoredNoteState({
	getVaultHandle: () => vaultHandle,
...
let selectedFilePinned = $derived(selectedFile ? storedNotes.pinnedNotePaths.includes(selectedFile.path) : false);
let pinnedNotes = $derived(
	getStoredNoteRecords(
		storedNotes.pinnedNotePaths.filter((path) => path !== selectedFile?.path),
		vaultIndex.recordsByPath
	)
);
let recentNotes = $derived(
	getStoredNoteRecords(
		storedNotes.recentNotePaths.filter(
			(path) => path !== selectedFile?.path && !storedNotes.pinnedNotePaths.includes(path)
		),
		vaultIndex.recordsByPath
	)
);
let baseViews = $derived(selectedFile?.extension === '.base' ? getBaseViews(selectedContent) : []);
let selectedCollection = $derived<ResolvedCollection | null>(
	resolveSelectedCollection(selectedFile, selectedContent, vaultIndex, selectedCollectionViewIndex)
);
...
let collectionKanbanGroups = $derived(getKanbanGroupsForView(selectedCollection, collectionRecords, collectionKanbanGroupBy));
let collectionTimelineItems = $derived(getTimelineItemsForView(selectedCollection, collectionRecords, collectionTimelineDateField));
let collectionSummaries = $derived(getCollectionSummariesForView(selectedCollection, collectionRecords));
const toggleSelectedPin = () => storedNotes.toggleSelectedPin(selectedRecord);
const shellActionContext = {
	get commandPaletteOpen() { return commandPaletteOpen; },
	set commandPaletteOpen(value) { commandPaletteOpen = value; },
	get commandPaletteQuery() { return commandPaletteQuery; },
	set commandPaletteQuery(value) { commandPaletteQuery = value; },
	get collectionCellEdit() { return collectionCellEdit; },
	set collectionCellEdit(value) { collectionCellEdit = value; },
	get collectionFilter() { return collectionFilter; },
	set collectionFilter(value) { collectionFilter = value; },
	get collectionRecordCreationError() { return collectionRecordCreationError; },
	get collectionRecords() { return collectionRecords; },
	get collectionSortColumn() { return collectionSortColumn; },
	set collectionSortColumn(value) { collectionSortColumn = value; },
	get collectionSortDirection() { return collectionSortDirection; },
	set collectionSortDirection(value) { collectionSortDirection = value; },
	get collectionSummaries() { return collectionSummaries; },
	...
	getErrorMessage,
	loadStoredNoteLists: storedNotes.loadStoredNoteLists,
	pruneSelectedPublicPublishProfile,
	pruneStoredNoteLists: storedNotes.pruneStoredNoteLists,
	recordRecentNote: storedNotes.recordRecentNote,
	...
	replaceStoredNotePath: storedNotes.replaceStoredNotePath,
	requestInlineFileCreate: requestState.requestInlineFileCreate,
	requestForm: requestState.requestForm,
	requestText: requestState.requestText,
