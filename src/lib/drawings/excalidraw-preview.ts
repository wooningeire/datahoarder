...
export const createExcalidrawNoteDraft = (title: string): ExcalidrawNoteDraft => {
...
export const renderExcalidrawNotePreview = (content: string) => {
...
export const parseExcalidrawScene = (content: string): ExcalidrawScene | null => {
...
export const renderExcalidrawSvg = (scene: ExcalidrawScene) => {
...
const getDrawingSection = (content: string) => {
...
const getJsonFenceContent = (content: string) => {
...
const getJsonObjectText = (content: string) => {
...
const renderElement = (element: ExcalidrawElement, markerId: string) => {
...
const renderTextElement = (element: ExcalidrawElement, color: string, opacity: number) => {
    const x = element.x ?? 0;
    const y = element.y ?? 0;
...
const getSceneBounds = (elements: ExcalidrawElement[]): Bounds => {
...
const getAbsolutePoints = (element: ExcalidrawElement) => {
...
const getTransform = (element: ExcalidrawElement) => {
...
const isRenderableElement = (element: unknown): element is ExcalidrawElement => {
    return Boolean(element) && typeof element === "object";
};

const isSupportedElement = (element: ExcalidrawElement) => {
    return ["arrow", "diamond", "ellipse", "freedraw", "line", "rectangle", "text"].includes(element.type ?? "");
};

const isDeletedElement = (element: ExcalidrawElement) => {
